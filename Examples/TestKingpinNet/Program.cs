...
namespace TestKingpinNet
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var runUrlArgument = runUrlCommand.AddArgument("url", "the url to run").IsRequired().IsUrl().FileExists().DirectoryExists();
            var runUrlFlag = runUrlCommand.AddFlag("flag", "a flag").Short('f').IsInt().IsUrl().IsIp().IsEnum(typeof(EnumType)).IsDuration().IsTcp().IsFloat().Default("5s");
            var runUrlSwitch = runUrlCommand.AddFlag("switch", "a switch").IsBool();
